...
def processRoutine():
    ...
    # 构建客户端
    if args.dev_build:
        build_apk.buildPackage("cn", "ChinaJoy", "dev")
    if args.dev_upload_assetbundle:
        build_apk.uploadAssetBundle("cn", "ChinaJoy", "dev")
    if args.dev_upload_package:
        build_apk.uploadPackage("cn", "ChinaJoy", "dev")

    if args.beta_build:
        build_apk.buildPackage("cn", "ChinaJoy", "beta")
    if args.beta_upload_assetbundle:
        build_apk.uploadAssetBundle("cn", "ChinaJoy", "beta")
    if args.beta_upload_package:
        build_apk.uploadPackage("cn", "ChinaJoy", "beta")

    if args.release_build:
        build_apk.buildPackage("cn", "ChinaJoy", "release")
    if args.release_upload_assetbundle:
        build_apk.uploadAssetBundle("cn", "ChinaJoy", "release")
    if args.release_upload_package:
        build_apk.uploadPackage("cn", "ChinaJoy", "release")
