...
def processRoutine():
    ...
    # 构建客户端
    if args.dev_build:
        build_ipa.buildPackage("cn", "ChinaJoy", "dev")
    if args.dev_upload_assetbundle:
        build_ipa.uploadAssetBundle("cn", "ChinaJoy", "dev")
    if args.dev_upload_package:
        build_ipa.uploadPackage("cn", "ChinaJoy", "dev")

    if args.beta_build:
        build_ipa.buildPackage("cn", "ChinaJoy", "beta")
    if args.beta_upload_assetbundle:
        build_ipa.uploadAssetBundle("cn", "ChinaJoy", "beta")
    if args.beta_upload_package:
        build_ipa.uploadPackage("cn", "ChinaJoy", "beta")

    if args.release_build:
        build_ipa.buildPackage("cn", "ChinaJoy", "release")
    if args.release_upload_assetbundle:
        build_ipa.uploadAssetBundle("cn", "ChinaJoy", "release")
    if args.release_upload_package:
        build_ipa.uploadPackage("cn", "ChinaJoy", "release")
