...
public class BASEConfigTable
{
  ...
  public List<BASEConfigElement> GetAllElement(Predicate<BASEConfigElement> matchCB = null)
	...
	public bool Load()
	{
		...
		{
			Debug.Log("配置文件[BASEConfig.bin]未找到");
			return false;
		}
	...
	public bool LoadBin(byte[] binContent)
	{
		...
		if(vecLine.Count != 4)
		{
			Debug.Log("BASEConfig.csv中列数量与生成的代码不匹配!");
			return false;
		}
		if(vecLine[0]!="ID"){Debug.Log("BASEConfig.csv中字段[ID]位置不对应"); return false; }
		if(vecLine[1]!="Name"){Debug.Log("BASEConfig.csv中字段[Name]位置不对应"); return false; }
		if(vecLine[2]!="Args1"){Debug.Log("BASEConfig.csv中字段[Args1]位置不对应"); return false; }
		if(vecLine[3]!="Args2"){Debug.Log("BASEConfig.csv中字段[Args2]位置不对应"); return false; }
