...
public class BaseAIElement
{
	...
	public int PatrolSpeed;      	//巡逻速度	m/s
	public int RaidSpeed;        	//追击速度	m/s
...
public class BaseAITable
{
  ...
  public List<BaseAIElement> GetAllElement(Predicate<BaseAIElement> matchCB = null)
	...
	public bool LoadBin(byte[] binContent)
	{
		...
		if(vecLine[0]!="baseAiId"){Debug.Log("BaseAI.csv中字段[baseAiId]位置不对应"); return false; }
		if(vecLine[1]!="Comment"){Debug.Log("BaseAI.csv中字段[Comment]位置不对应"); return false; }
		if(vecLine[2]!="canMove"){Debug.Log("BaseAI.csv中字段[canMove]位置不对应"); return false; }
		if(vecLine[3]!="attackType"){Debug.Log("BaseAI.csv中字段[attackType]位置不对应"); return false; }
		if(vecLine[4]!="aiCd"){Debug.Log("BaseAI.csv中字段[aiCd]位置不对应"); return false; }
		if(vecLine[5]!="noAttack"){Debug.Log("BaseAI.csv中字段[noAttack]位置不对应"); return false; }
		if(vecLine[6]!="patrolRate"){Debug.Log("BaseAI.csv中字段[patrolRate]位置不对应"); return false; }
		if(vecLine[7]!="followPortal"){Debug.Log("BaseAI.csv中字段[followPortal]位置不对应"); return false; }
		if(vecLine[8]!="counterAttack"){Debug.Log("BaseAI.csv中字段[counterAttack]位置不对应"); return false; }
		if(vecLine[9]!="SkillPriority"){Debug.Log("BaseAI.csv中字段[SkillPriority]位置不对应"); return false; }
		if(vecLine[10]!="GuardType"){Debug.Log("BaseAI.csv中字段[GuardType]位置不对应"); return false; }
		if(vecLine[11]!="GuardScope"){Debug.Log("BaseAI.csv中字段[GuardScope]位置不对应"); return false; }
		if(vecLine[12]!="PatrolSpeed"){Debug.Log("BaseAI.csv中字段[PatrolSpeed]位置不对应"); return false; }
		if(vecLine[13]!="RaidSpeed"){Debug.Log("BaseAI.csv中字段[RaidSpeed]位置不对应"); return false; }
		if(vecLine[14]!="BloodReturn"){Debug.Log("BaseAI.csv中字段[BloodReturn]位置不对应"); return false; }
		if(vecLine[15]!="InterveneSkill"){Debug.Log("BaseAI.csv中字段[InterveneSkill]位置不对应"); return false; }
		if(vecLine[16]!="ExpandAI"){Debug.Log("BaseAI.csv中字段[ExpandAI]位置不对应"); return false; }
