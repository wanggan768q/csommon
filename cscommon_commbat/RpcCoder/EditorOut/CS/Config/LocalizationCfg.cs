...
public class LocalizationTable
{
  ...
  public List<LocalizationElement> GetAllElement(Predicate<LocalizationElement> matchCB = null)
	...
	public bool Load()
	{
		...
		{
			Debug.Log("配置文件[Localization.bin]未找到");
			return false;
		}
	...
	public bool LoadBin(byte[] binContent)
	{
		...
		if(vecLine.Count != 4)
		{
			Debug.Log("Localization.csv中列数量与生成的代码不匹配!");
			return false;
		}
		if(vecLine[0]!="id"){Debug.Log("Localization.csv中字段[id]位置不对应"); return false; }
		if(vecLine[1]!="key"){Debug.Log("Localization.csv中字段[key]位置不对应"); return false; }
		if(vecLine[2]!="cn"){Debug.Log("Localization.csv中字段[cn]位置不对应"); return false; }
		if(vecLine[3]!="en"){Debug.Log("Localization.csv中字段[en]位置不对应"); return false; }
