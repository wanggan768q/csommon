...
public:
	...
	string HtmlDescHeader()
	{
		...
		htmlBuff += "<li>SlotId(槽位Id) [sint32]</li>\r\n";
		htmlBuff += "<li>SlotType(槽位类别(0上古，1其他)) [sint32]</li>\r\n";
		htmlBuff += "<li>SlotSkillId(聚灵技能ID) [sint32]</li>\r\n";
	...
	string ToHtml()
	{
		...
		htmlBuff += tmpLine;
		tmpLine.Fmt("<li>SlotType：%di</li>\r\n",m_SlotType);
		htmlBuff += tmpLine;
		tmpLine.Fmt("<li>SlotSkillId：%di</li>\r\n",m_SlotSkillId);
		htmlBuff += tmpLine;
...
public:
	...
	string HtmlDescHeader()
	{
		...
		htmlBuff += "<li>CurExp(当前经验) [sint32]</li>\r\n";
		htmlBuff += "<li>EquipScore(装备评分) [sint32]</li>\r\n";
		htmlBuff += "<li>StarLevel(星级) [sint32]</li>\r\n";
		htmlBuff += "<li>GatherSpriteSlot(聚灵槽位) [SlotInfo] Array</li>\r\n";
		htmlBuff += "<li>TailsManLevel(法宝等级) [sint32]</li>\r\n";
		htmlBuff += "<li>TalismanAttr(法宝属性) [sint32] Array</li>\r\n";
	...
	string ToHtml()
	{
		...
		tmpLine.Fmt("<li>TemplateId：%di</li>\r\n",m_TemplateId);
		htmlBuff += tmpLine;
		tmpLine.Fmt("<li>CurExp：%di</li>\r\n",m_CurExp);
		htmlBuff += tmpLine;
		tmpLine.Fmt("<li>EquipScore：%di</li>\r\n",m_EquipScore);
		htmlBuff += tmpLine;
		tmpLine.Fmt("<li>StarLevel：%di</li>\r\n",m_StarLevel);
		htmlBuff += tmpLine;
		htmlBuff += "<li>GatherSpriteSlot：</li>\r\n";
		if( m_GatherSpriteSlot.size()>0) htmlBuff += m_GatherSpriteSlot[0].HtmlDescHeader();
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n";
		for( int i=0; i<(int)m_GatherSpriteSlot.size(); i++){
			tmpLine.Fmt("%d",i);
			htmlBuff += "<tr><td>"+ tmpLine+"</td><td>"+m_GatherSpriteSlot[i].ToHtml().c_str() +"</td></tr>";
		}
		htmlBuff += "</table></div>";
		tmpLine.Fmt("<li>TailsManLevel：%di</li>\r\n",m_TailsManLevel);
		htmlBuff += tmpLine;
		htmlBuff += "<li>TalismanAttr：</li>\r\n";
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n<tr>\r\n";
		for( int i=0; i<(int)m_TalismanAttr.size(); i++){
		...
		if( (m_TalismanAttr.size() +1)%10 != 0 ) htmlBuff += "</tr>";
		htmlBuff += "</table></div>";
...
public:
	...
	{
		string htmlBuff = "<div style=\"padding-left:30px\">\r\n";
		htmlBuff += "<li>GridArray(背包格子数组) [GridInfo] Array</li>\r\n";
		htmlBuff += "<li>EquipAllData(装备全部数据) [BagEquipObj] Array</li>\r\n";
		htmlBuff += "<li>WearEquipContainer(穿戴装备容器) [GridInfo] Array</li>\r\n";
		htmlBuff += "<li>StorageBagGrid(仓库) [GridInfo] Array</li>\r\n";
		htmlBuff += "<li>StorageBank(仓库金钱) [sint32]</li>\r\n";
		htmlBuff += "<li>TalismanData(法宝数据) [TalismanObj] Array</li>\r\n";
		htmlBuff += "<li>TalismanForeverAttr(装备过的法宝，可获得永久属性加成) [sint32] Array</li>\r\n";
		htmlBuff += "<li>AutoPickup(自动拾取设置) [KeyValue2IntBool] Array</li>\r\n";
		htmlBuff += "<li>UseItemDay(每天使用的道具) [KeyValue2IntInt] Array</li>\r\n";
		htmlBuff += "<li>UseItemWeek(每周使用的道具) [KeyValue2IntInt] Array</li>\r\n";
	...
	string ToHtml()
	{
		string htmlBuff = "<div style=\"padding-left:30px\">\r\n";
		TStr tmpLine;
		htmlBuff += "<li>GridArray：</li>\r\n";
		if( m_GridArray.size()>0) htmlBuff += m_GridArray[0].HtmlDescHeader();
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n";
		for( int i=0; i<(int)m_GridArray.size(); i++){
			tmpLine.Fmt("%d",i);
			htmlBuff += "<tr><td>"+ tmpLine+"</td><td>"+m_GridArray[i].ToHtml().c_str() +"</td></tr>";
		}
		htmlBuff += "</table></div>";
		...
		htmlBuff += "</table></div>";
		htmlBuff += "<li>WearEquipContainer：</li>\r\n";
		if( m_WearEquipContainer.size()>0) htmlBuff += m_WearEquipContainer[0].HtmlDescHeader();
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n";
		for( int i=0; i<(int)m_WearEquipContainer.size(); i++){
			tmpLine.Fmt("%d",i);
			htmlBuff += "<tr><td>"+ tmpLine+"</td><td>"+m_WearEquipContainer[i].ToHtml().c_str() +"</td></tr>";
		}
		htmlBuff += "</table></div>";
		htmlBuff += "<li>StorageBagGrid：</li>\r\n";
		if( m_StorageBagGrid.size()>0) htmlBuff += m_StorageBagGrid[0].HtmlDescHeader();
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n";
		for( int i=0; i<(int)m_StorageBagGrid.size(); i++){
			tmpLine.Fmt("%d",i);
			htmlBuff += "<tr><td>"+ tmpLine+"</td><td>"+m_StorageBagGrid[i].ToHtml().c_str() +"</td></tr>";
		}
		htmlBuff += "</table></div>";
		tmpLine.Fmt("<li>StorageBank：%di</li>\r\n",m_StorageBank);
		htmlBuff += tmpLine;
		htmlBuff += "<li>TalismanData：</li>\r\n";
		if( m_TalismanData.size()>0) htmlBuff += m_TalismanData[0].HtmlDescHeader();
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n";
		for( int i=0; i<(int)m_TalismanData.size(); i++){
			tmpLine.Fmt("%d",i);
			htmlBuff += "<tr><td>"+ tmpLine+"</td><td>"+m_TalismanData[i].ToHtml().c_str() +"</td></tr>";
		}
		htmlBuff += "</table></div>";
		htmlBuff += "<li>TalismanForeverAttr：</li>\r\n";
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n<tr>\r\n";
		for( int i=0; i<(int)m_TalismanForeverAttr.size(); i++){
			tmpLine.Fmt("<td>%di</td>\r\n",m_TalismanForeverAttr[i]);
			htmlBuff += tmpLine;
			if((i+1)%10==0) htmlBuff += "</tr>\r\n<tr>";
		}
		if( (m_TalismanForeverAttr.size() +1)%10 != 0 ) htmlBuff += "</tr>";
		htmlBuff += "</table></div>";
		htmlBuff += "<li>AutoPickup：</li>\r\n";
		if( m_AutoPickup.size()>0) htmlBuff += m_AutoPickup[0].HtmlDescHeader();
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n";
		for( int i=0; i<(int)m_AutoPickup.size(); i++){
			tmpLine.Fmt("%d",i);
			htmlBuff += "<tr><td>"+ tmpLine+"</td><td>"+m_AutoPickup[i].ToHtml().c_str() +"</td></tr>";
		}
		htmlBuff += "</table></div>";
		htmlBuff += "<li>UseItemDay：</li>\r\n";
		if( m_UseItemDay.size()>0) htmlBuff += m_UseItemDay[0].HtmlDescHeader();
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n";
		for( int i=0; i<(int)m_UseItemDay.size(); i++){
			tmpLine.Fmt("%d",i);
			htmlBuff += "<tr><td>"+ tmpLine+"</td><td>"+m_UseItemDay[i].ToHtml().c_str() +"</td></tr>";
		}
		htmlBuff += "</table></div>";
		htmlBuff += "<li>UseItemWeek：</li>\r\n";
		if( m_UseItemWeek.size()>0) htmlBuff += m_UseItemWeek[0].HtmlDescHeader();
		htmlBuff +="<div style=\"padding-left:15px\"><table border=1>\r\n";
		for( int i=0; i<(int)m_UseItemWeek.size(); i++){
			tmpLine.Fmt("%d",i);
			htmlBuff += "<tr><td>"+ tmpLine+"</td><td>"+m_UseItemWeek[i].ToHtml().c_str() +"</td></tr>";
		}
		htmlBuff += "</table></div>";
